...
if (!empty($_POST['is_submit_event'])) {
    ...
    if (!empty($event_id)) {
        $query = "UPDATE tblevent SET event_name='$event_name', event_start_date='$event_start_date', event_end_date='$event_end_date', event_start_time='$event_start_time', event_end_time='$event_end_time', user_id='$user_id' WHERE event_id='$event_id' ";
        $query_run = mysqli_query($conn, $query);
        if ($query_run) {
            echo json_encode([
                'ito' => ''
        ...
        $query = "INSERT INTO tblevent (event_name, event_start_date, event_end_date, event_start_time, event_end_time, instructor, user_id) VALUES ( '$event_name', '$event_start_date', '$event_end_date', '$event_start_time', '$event_end_time', '$instructor', '$user_id' )";
        $query_run = mysqli_query($conn, $query);
...
if (!empty($_POST['is_get_user'])) {
    ...
    if(!empty($date_start) && !empty($date_end)){
        ...
        while ($row = mysqli_fetch_assoc($result_user)) {
            $array_user[] = $row['user_id'];
        }
    
        $user_ids = implode(", ",$array_user);
    }


    $query = "select * from user where isactive=1";
    if(!empty($array_user)){
        $query .= " AND id not in ($user_ids)";
    }
    $result = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_assoc($result)) {
