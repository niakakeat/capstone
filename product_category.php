...
<body>
    ...
    <div class="container">
        <div class="panel panel-primary">
            <div class="panel-heading" style="background-color: black;">Product Category </div>
            <div class="panel-body">
                <button type="button" id="add_category"><i class="fas fa-plus"></i> Add Category</button>
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                            <tr style="<?= $row['status'] != 1 ? 'text-decoration: line-through; background-color: pink; color: black;' : '' ?>">
                                ...
                                <td><button type="button" category_id="<?= $row['id']; ?>" style="background-color: #00b6ff;" class="edit_category">Edit</button></td>
                                <?php if ($row['status'] == 1) : ?>
                                    <td><button type="button" category_id="<?= $row['id']; ?>" style="background-color: #d9534f;" class="deactivate_category">Deactivate</button></td>
                                <?php else : ?>
                                    <td><button type="button" category_id="<?= $row['id']; ?>" style="background-color: #3cb371;" class="activate_category">Activate</button></td>
                                <?php endif; ?>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="config/product_category.php" method="POST">
                        <input type="hidden" name="category_id" id="category_id">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="">Category Name</label>
                                <input type="text" name="name" id="Category_name" class="form-control inputs">
                            </div>

                            <div class="col-md-12 mb-3">
                                <button type="button" name="submit_category" id="submit_category" value="1" class="btn btn-primary">Submit</button>
                            </div>
                        </div>
                    </form>
                </div>
